...
extension PendulumInterface {
    ...
    struct Counter: ARTraceableParagraphContainer {
        ...
        unowned let pendulumInterface: PendulumInterface
        private var counts: Counts = .init()
        
        var buttons: [ARInterfaceElement]
        private var highlightedButton: CachedParagraph?
        
        subscript(index: CachedParagraph) -> ARInterfaceElement {
            get { buttons[index.rawValue] }
            set { buttons[index.rawValue] = newValue }
        }
        ...
        init(pendulumInterface: PendulumInterface) {
            ...
            buttons = .init(capacity: CachedParagraph.allCases.count)
            
            for panel in CachedParagraph.allCases {
                buttons.append(panel.interfaceElement)
            }
        }
        
        mutating func resetSize() {
            for button in CachedParagraph.allCases {
                self[button] = button.interfaceElement
            }
        }
        ...
        mutating func resetHighlighting() {
            if let highlightedButton = highlightedButton {
                self[highlightedButton].isHighlighted = false
...
fileprivate protocol PendulumInterfaceCircularButton: ARParagraph { }

extension PendulumInterface.CircularButton {
    
    static var width: Float { 0.8 * 0.10 }
    static var height: Float { 0.8 * 0.10 }
    ...
    static func generateInterfaceElement(type: PendulumInterface.Counter.CachedParagraph) -> ARInterfaceElement {
        var paragraph = PendulumInterface.Counter.createParagraph(type)
        ...
        return .init(position: .zero, forwardDirection: [0, 0, 1], orthogonalUpDirection: [0, 1, 0],
                     ...
                     depth: 0.8 * 0.05 * scale, radius: .greatestFiniteMagnitude,
