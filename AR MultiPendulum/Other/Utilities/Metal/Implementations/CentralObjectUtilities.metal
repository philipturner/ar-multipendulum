...
#include "../CentralObjectUtilities.metal"
...
bool CentralObjectUtilities::shouldCull(threadgroup void *tg_8bytes,
...
bool CentralObjectUtilities::Serial::shouldCull(thread float4 *projectedVertices)
{
...
namespace InternalCentralObjectUtilities {
...
using namespace InternalCentralObjectUtilities;

LOD CentralObjectUtilities::getLOD(threadgroup void *tg_64bytes,
                                   float4x4 modelToWorldTransform,
                                   float4x4 worldToModelTransform,
                                   ...
                                   ushort thread_id)
{
    ...
    ushort transformIndex = isMR ? quadgroup_id : 0;
...
using namespace InternalCentralObjectUtilities::Serial;

LOD CentralObjectUtilities::Serial::getLOD(float4x4 modelToWorldTransform,
                                           float4x4 worldToModelTransform,
                                           constant float4x4 *worldToCameraTransforms,
                                           constant float3   *cameraPositions,
                                           bool isMR,
                                           ...
                                           float3 objectScaleHalf, float3 objectPosition)
{
    ushort numTransforms = select(1, 2, isMR);
