...
final class SceneTexelRasterizer: DelegateSceneRenderer {
    ...
    var oldTriangleMarkBuffer: MultiLevelBuffer<TriangleMarkLevel> { sceneTexelManager.oldTriangleMarkBuffer }
    var smallTriangleColorBuffer: MultiLevelBuffer<SmallColorLevel> { sceneTexelManager.smallTriangleColorBuffer }
    var largeTriangleColorBuffer: MultiLevelBuffer<LargeColorLevel> { sceneTexelManager.largeTriangleColorBuffer }
    
    enum TriangleDataLevel: UInt8, MultiLevelBufferLevel {
        case texelCount
        case texelOffset
    ...
    enum TexelLevel: UInt8, MultiLevelBufferLevel {
        case luma
        case chroma
    ...
    var oldTriangleDataBuffer: MultiLevelBuffer<TriangleDataLevel>
    var bridgeBuffer: MultiLevelBuffer<BridgeLevel>
    var texelBuffer: MultiLevelBuffer<TexelLevel>
    ...
    init(sceneRenderer: SceneRenderer, library: MTLLibrary) {
        ...
        triangleDataBuffer    = device.makeMultiLevelBuffer(capacity: triangleCapacity)
        oldTriangleDataBuffer = device.makeMultiLevelBuffer(capacity: triangleCapacity)
        bridgeBuffer          = device.makeMultiLevelBuffer(capacity: triangleCapacity, options: .storageModeShared)
        texelBuffer           = device.makeMultiLevelBuffer(capacity: texelCapacity)
...
extension SceneTexelRasterizer: BufferExpandable {
    ...
    private func ensureTriangleDataCapacity(capacity: Int) {
        ...
        triangleDataBuffer.expandCapacity(device: device, capacity: capacity)
    }
