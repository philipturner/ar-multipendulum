...
final class SceneMeshReducer: DelegateSceneRenderer {
    ...
    typealias SmallSectorHashLevel = SceneMeshMatcher.SmallSectorLevel
    var smallSectorHashBuffer: MultiLevelBuffer<SmallSectorHashLevel> { sceneMeshMatcher.newSmallSectorBuffer }
    ...
    enum SectorIDLevel: UInt8, MultiLevelBufferLevel {
    ...
    var currentSectorIDBuffer: MultiLevelBuffer<SectorIDLevel>
    var pendingSectorIDBuffer: MultiLevelBuffer<SectorIDLevel>
    ...
    init(sceneRenderer: SceneRenderer, library: MTLLibrary) {
        ...
        uniformBuffer = device.makeMultiLevelBuffer(capacity: meshCapacity,   options: .storageModeShared)
        bridgeBuffer  = device.makeMultiLevelBuffer(capacity: vertexCapacity, options: .storageModeShared)
        
        currentSectorIDBuffer = device.makeMultiLevelBuffer(capacity: triangleCapacity)
        pendingSectorIDBuffer = device.makeMultiLevelBuffer(capacity: triangleCapacity)
    ...
    fileprivate func makeReduceSubmeshesPipeline() -> MTLComputePipelineState {
        // This file has no target membership so that it isn't included in the app executable.
        // To access it through this function, include it in this project's build target.
        let reduceSubmeshesLibraryURL = Bundle.main.url(forResource: "ReduceSubmeshesLibrary", withExtension: "metallib")!
        let reduceSubmeshesLibrary = try! device.makeLibrary(URL: reduceSubmeshesLibraryURL)
        
        return reduceSubmeshesLibrary.makeComputePipeline(Self.self, name: "reduceSubmeshes")
    }
