...
final class ThirdSceneSorter: DelegateSceneSorter {
    ...
    enum MicroSector512thLevel: UInt8, MultiLevelBufferLevel {
        case counts
        case offsets
    ...
    var microSector512thBuffer: MultiLevelBuffer<MicroSector512thLevel>
    var bridgeBuffer: MultiLevelBuffer<BridgeLevel>
    var microSectorBuffer: MultiLevelBuffer<MicroSectorLevel>
    ...
    init(sceneSorter: SceneSorter, library: MTLLibrary) {
        ...
        microSector512thBuffer = device.makeMultiLevelBuffer(capacity: smallSectorCapacity)
        bridgeBuffer           = device.makeMultiLevelBuffer(capacity: smallSectorCapacity, options: .storageModeShared)
        microSectorBuffer      = device.makeMultiLevelBuffer(capacity: microSectorCapacity)
