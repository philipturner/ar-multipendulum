...
extension SceneMeshMatcher {
    ...
    func prepareForMeshMatch() {
        ...
        debugLabel {
            oldSmallSectorBuffer.label = "Old " + SmallSectorLevel.bufferLabel
            newSmallSectorBuffer.label = "New " + SmallSectorLevel.bufferLabel
        }
    ...
    func initializeOldBuffers() {
        ...
        oldMicroSector512thBuffer = device.makeMultiLevelBuffer(capacity: newMicroSector512thBuffer.capacity)
        oldNanoSector512thBuffer  = device.makeMultiLevelBuffer(capacity: newNanoSector512thBuffer.capacity, options: .storageModeShared)
        oldVertexMapBuffer        = device.makeMultiLevelBuffer(capacity: newVertexMapBuffer.capacity)
