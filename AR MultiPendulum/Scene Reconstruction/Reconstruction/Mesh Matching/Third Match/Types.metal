...
public:
    ...
    template <bool doingExpandedBounds>
    void copyCoords(Location other, ushort index)
    {
        if (doingExpandedBounds)
        {
    ...
    void increaseCoord(ushort index, uchar changeMagnitude)
    {
        ...
        if (nanoSector[index] >= 8)
        {
            ...
            if (doingExpandedBounds && microSector[index] >= 8)
            {
                microSector[index]= 0;
                ++smallSector[index];
            }
...
private:
    bool incrementBody()
    {
        ...
        {
            iterator.increaseCoord<doingExpandedBounds>(i, granularity);
            if (iterator.lessOrEqual<doingExpandedBounds>(end, i)) { return true; }
            
            iterator.copyCoords<doingExpandedBounds>(start, i);
        }
        
        iterator.increaseCoord<doingExpandedBounds>(2, granularity);
        return iterator.lessOrEqual<doingExpandedBounds>(end, 2);
    }
    ...
    void processGranularityChange()
    {
        ...
        if (initializingIterator || doingExpandedBounds)
        {
            iterator = start;
        }
        else
