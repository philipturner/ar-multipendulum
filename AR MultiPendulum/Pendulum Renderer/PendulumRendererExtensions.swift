...
extension PendulumRenderer: CustomRenderer {
    
    func updateResources() {
        doingTwoSidedPendulums = (renderer.coordinator as! Coordinator).doingTwoSidedPendulums
        ...
        if shouldRenderToDisplay {
            meshConstructor.updateResources()
        }
        
        if shouldRenderTrajectory {
            failureTrajectory.updateObjects(pendulumRenderer: self, frameID: lastFrameID)
            
            centralRenderer.render(objects: failureTrajectory.jointObjects, desiredLOD: 64)
            centralRenderer.render(objects: failureTrajectory.rectangleObjects)
        }
        
        pendulumInterface.updateResources()
        
        createStandObjects()
        centralRenderer.render(objects: standObjects)
    }
    
    func drawGeometry(renderEncoder: ARMetalRenderCommandEncoder) {
        meshConstructor.drawGeometry(renderEncoder: renderEncoder)
    }
