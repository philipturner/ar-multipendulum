...
extension LensDistortionCorrector {
    ...
    struct LensDistortionUniforms {
        ...
        init(corrector: LensDistortionCorrector, clearingFramebuffer: Bool) {
            ...
            @inline(__always)
            func getCoefficients(_ base: Float, _ proportions: simd_float2) -> simd_packed_half3 {
                let multipliers = simd_float3(proportions[0] * compressionRatio,
                                                               compressionRatio,
                                              proportions[1] * compressionRatio)
                
                return simd_packed_half3(base * multipliers)
            }
            
            var storedSettings: LensDistortionCorrector.StoredSettings { corrector.storedSettings }
            
            k1_coefficients = getCoefficients(storedSettings.k1, storedSettings.k1_proportions)
            k2_coefficients = getCoefficients(storedSettings.k2, storedSettings.k2_proportions)
