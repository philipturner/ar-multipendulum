...
final class LensDistortionCorrector: DelegateUserSettings {
    ...
    enum UniformLevel: UInt8, MultiLevelBufferLevel {
        ...
        func getSize(capacity: Int) -> Int {
            switch self {
            case .vrrMap: return capacity * Renderer.numRenderBuffers * MemoryLayout<UInt8>.stride
            }
        }
    }
    ...
    var mixedRealityMSAATexture: MTLTexture!
    var mixedRealityResolveTexture: MTLTexture!
    var mixedRealityDepthStencilTexture: MTLTexture!
    ...
    init(userSettings: UserSettings, library: MTLLibrary) {
        ...
        uniformBuffer = device.makeMultiLevelBuffer(capacity: 32768, options: [.cpuCacheModeWriteCombined, .storageModeShared])
